...
pandas [@pandas] (McKinney 2010), altair [@altair], click [@click], 
...
We present the standard deviations in @fig-horse_pop.
...
Additionally, note that in @tbl-horse-sd we consider the sample standard deviation of the number of horses during the same time span as @fig-horse_pop.

![Horse populations for the province with the largest standard deviation](../results/horse_pops_plot_largest_sd.png){#fig-horse_dev}


```{python}
province = horses_sd_table.iloc[0,0]
```

In @fig-horse_dev we zoom in and look at the province of `{python} province`, which had the largest spread of values in terms of standard deviation.
